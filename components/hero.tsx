...
import {
  Scan, Star, UtensilsCrossed, Cpu, Zap, Edit2, Check, X, Save, FolderTree,
  Settings, Clock, Key, Shield, ChefHat, ArrowRight,
} from "lucide-react";
...
export function Hero() {
  ...
  useEffect(() => {
    setAdminMenu(adminMenuData.map((item) => ({
      ...item, name: t(item.nameKey), category: t(item.categoryKey),
    })));
  }, [language, t]);

  useEffect(() => {
    setCategories(categoriesData.map((cat) => ({
      ...cat, name: t(cat.nameKey), nameEn: t(cat.nameEnKey),
    })));
  }, [language, t]);
  ...
  return (
    <>
      {/* HERO */}
      <section className="relative w-full h-[100vh] max-h-[900px] min-h-[500px] overflow-hidden">
        ...
        <div className="relative z-10 w-full h-full flex items-center justify-center px-4">
          <div className="flex flex-col items-center text-center max-w-4xl">
            ...
            <div className="h-14 sm:h-20 mt-4">
              <p className={`text-lg sm:text-xl md:text-2xl lg:text-3xl text-white font-semibold transition-all duration-500 transform ${showPhrase ? "opacity-100 translate-y-0" : "opacity-0 -translate-y-4"}`}>{phrases[currentPhraseIndex]}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="relative py-16 sm:py-20 md:py-24 overflow-hidden bg-black">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 justify-items-center">
            {[1, 2].map((_, i) => (
              <div key={i} className="relative mx-auto w-72 sm:w-80 md:w-96">
                ...
                <div className="relative bg-gradient-to-br from-zinc-900 to-black rounded-[2.5rem] sm:rounded-[3rem] p-2 sm:p-3 shadow-2xl border border-red-500/30">
                  ...
                  <div className="bg-black rounded-[2rem] sm:rounded-[2.5rem] overflow-hidden h-[500px] sm:h-[550px] md:h-[600px]">
                    <div className="bg-gradient-to-r from-red-600/20 to-purple-700/20 p-2 sm:p-3 pt-10 sm:pt-12 border-b border-red-500/20">
                      <span className="text-white font-semibold text-[10px] sm:text-xs">{i === 0 ? t("demo.productos") : t("demo.categorias")}</span>
                      <p className="text-[8px] text-zinc-500 mt-1 text-center">{i === 0 ? t("demo.productosHint") : t("demo.categoriasHint")}</p>
                    </div>
                    <div className="p-2 sm:p-3 space-y-2 h-[420px] sm:h-[460px] md:h-[510px] overflow-y-auto custom-scrollbar">
                      {i === 0 ? adminMenu.map((item) => (
                        <div key={item.id} className="bg-gradient-to-r from-zinc-900/50 to-black/50 rounded-xl overflow-hidden border border-zinc-800">
                          <div className="flex gap-2 p-2 sm:p-3">
                            <div className="relative w-12 h-12 sm:w-14 sm:h-14 md:w-16 md:h-16 rounded-lg overflow-hidden flex-shrink-0 bg-zinc-800">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-red-400 font-bold text-xs">€{item.price}</span>
                            </div>
                          </div>
                        </div>
                      )) : categories.map((cat) => (
